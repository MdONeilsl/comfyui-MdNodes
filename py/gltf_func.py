...
#===================================================================================
def perform_save_gltf(double_size, color_tint, alpha, alpha_mode, alpha_cut,
    ...
    # Helper to embed an image into the GLB
    def save_and_embed(arr, name, force_rgba=False):
        ...
        # Disk save (optional, but original code did it)
    ...
    # Textures
    base_idx, orm_idx, ao_idx, normal_idx, emissive_idx = None, None, None, None, None

    if kwargs.get("color") is not None:
        base_idx = save_and_embed(_tensor_to_numpy(kwargs["color"]), "color", force_rgba=True)
    ...
    # Geometry
    pos = np.array([[-0.5, -0.5, 0], [0.5, -0.5, 0], [0.5, 0.5, 0], [-0.5, 0.5, 0]], np.float32)
...
# ---------- load logic ----------
def _empty_tensor(channels=3):
    return torch.zeros((1, 64, 64, channels), dtype=torch.float32)

def _safe_open_image(img_bytes):
    try:
...
def _load_buffers(gltf_json, gltf_dir, glb_bin_chunk):
    buffers = []
    for i, buffer_info in enumerate(gltf_json.get("buffers", [])):
        ...
        if not os.path.isfile(path):
            raise FileNotFoundError(f"Missing buffer file: {path}")
        with open(path, "rb") as f:
            buffers.append(f.read())
    return buffers

def _extract_buffer_view(gltf_json, buffers, buffer_view_index):
    ...
    if end > len(data):
        raise ValueError("BufferView exceeds buffer size")
    return data[byte_offset:end]

def _load_image_bytes(gltf_json, buffers, image_info, gltf_dir):
    if "bufferView" in image_info:
        return _extract_buffer_view(gltf_json, buffers, image_info["bufferView"])
...
def _load_texture_tensor(gltf_json, buffers, tex_index, gltf_dir, channels=3):
    if tex_index is None:
        return _empty_tensor(channels)
    ...
    try:
        ...
        img_bytes = _load_image_bytes(gltf_json, buffers, image_info, gltf_dir)
...
def _extract_channel(tensor, channel_index):
    if tensor.shape[-1] <= channel_index:
        return _empty_tensor(1)
    return tensor[..., channel_index:channel_index + 1].clone()

#===================================================================================
def perform_load_gltf(gltf_filename):
    filepath = os.path.join(folder_paths.get_input_directory(), gltf_filename)
        ...
        if magic == b"glTF":
            ...
            while f.tell() < length:
                ...
                if chunk_type == b"JSON":
                    try:
                        json_text = chunk_data.decode("utf-8").rstrip("\x00 \t\r\n")
                        gltf_json = json.loads(json_text)
                    except json.JSONDecodeError as e:
                        raise ValueError(f"Invalid GLB JSON chunk: {e}")
            ...
            try:
                ...
                raise ValueError(f"Invalid GLTF JSON: {e}")

    buffers = _load_buffers(gltf_json, gltf_dir, glb_bin_chunk)
    ...
    r, g, b, a = base_color
    color_tint = (to_byte(r) << 16) | (to_byte(g) << 8) | to_byte(b)
    alpha = float(a)

    emissive_factor = material.get("emissiveFactor", [0.0, 0.0, 0.0])
    er, eg, eb = emissive_factor
    emissive_color = (to_byte(er) << 16) | (to_byte(eg) << 8) | to_byte(eb)

    metallic_factor = float(pbr.get("metallicFactor", 1.0))
    roughness_factor = float(pbr.get("roughnessFactor", 1.0))
    alpha_mode = material.get("alphaMode", "OPAQUE")
    alpha_cut = float(material.get("alphaCutoff", 0.5))
    double_side = bool(material.get("doubleSided", False))

    color_idx = pbr.get("baseColorTexture", {}).get("index")
    metal_idx = pbr.get("metallicRoughnessTexture", {}).get("index")
    emissive_idx = material.get("emissiveTexture", {}).get("index")
    normal_idx = material.get("normalTexture", {}).get("index")
    occlusion_idx = material.get("occlusionTexture", {}).get("index")

    color_map = _load_texture_tensor(gltf_json, buffers, color_idx, gltf_dir, channels=4)
    metal_map = _load_texture_tensor(gltf_json, buffers, metal_idx, gltf_dir, channels=3)
    emissive_map = _load_texture_tensor(gltf_json, buffers, emissive_idx, gltf_dir, channels=3)
    normal_map = _load_texture_tensor(gltf_json, buffers, normal_idx, gltf_dir, channels=3)

    if occlusion_idx is not None:
        occlusion_rgb = _load_texture_tensor(gltf_json, buffers, occlusion_idx, gltf_dir, channels=3)
        occlusion_map = _extract_channel(occlusion_rgb, 0)
    else:
        occlusion_map = _extract_channel(metal_map, 0)
